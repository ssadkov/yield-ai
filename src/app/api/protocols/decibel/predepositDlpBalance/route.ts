...
/**
 * GET /api/protocols/decibel/predepositDlpBalance
 * Proxies to Decibel predeposits/positions/dlp API.
 * Doc: https://docs.decibel.trade/api-reference/predeposit-points/get-dlp-balance-for-a-user
 *
 * Query: address (required). Returns { account, balance } (balance as decimal string).
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const url = `${baseUrl}/api/v1/predeposits/positions/dlp?${params.toString()}`;
    ...
    console.error('[Decibel] predepositDlpBalance error:', error);
    return NextResponse.json(
      {
        success: false,
