...
/**
 * GET /api/protocols/decibel/tradeHistory
 * Proxies to Decibel trade_history API.
 * Doc: GET /api/v1/trade_history?account={address}&limit=&offset=&order_id=&market=
 */
export async function GET(request: NextRequest) {
  try {
    ...
    if (orderId != null && orderId !== '') params.set('order_id', orderId);
    if (market != null && market !== '') params.set('market', market);
    const url = `${baseUrl}/api/v1/trade_history?${params.toString()}`;
    ...
    console.error('[Decibel] tradeHistory error:', error);
    return NextResponse.json(
