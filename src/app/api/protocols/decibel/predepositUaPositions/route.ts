...
/**
 * GET /api/protocols/decibel/predepositUaPositions
 * Proxies to Decibel predeposits/positions/ua API (non-zero UA balances only).
 * Doc: https://docs.decibel.trade/api-reference/predeposit-points/get-ua-positions-for-a-user-non-zero-balances-onlycomputed-on-demand-from-predeposit_balance_events
 *
 * Query: address (required). Returns array of { deposit_index, balance }.
 */
export async function GET(request: NextRequest) {
  try {
    ...
    console.error('[Decibel] predepositUaPositions error:', error);
    return NextResponse.json(
      {
        success: false,
