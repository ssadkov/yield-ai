...
const VIEW_FUNCTION = `${EARNIUM_PACKAGE}::premium_staked_pool::claimable_rewards`;
...
export async function GET(request: NextRequest) {
  try {
    ...
    for (const pool of pools) {
      ...
      try {
        // Pass pool index as string to satisfy u64/u128 JSON encoding expectations
        const viewResp = await callView(VIEW_FUNCTION, [address, String(pool)]);
        // /v1/view returns an array of return values; take first
        rawResponse = Array.isArray(viewResp) ? viewResp[0] : viewResp;
      } catch (e) {
        rawResponse = { data: [] };
      }

      const entries = normalizeEntries(rawResponse);
      ...
      results.push({ pool, raw: rawResponse, rewards });
    }
